...
(function ($) {
  ...
  if( !$style_switcher.length && showSwitcher ) {
	...
	// Paving Leads: header + back-to-top used to run on every scroll event, and the back-to-top
	// check read $(document).height() (a forced layout) each time. One rAF-throttled listener now.
	var plScrollTicking = false;
	window.addEventListener('scroll', function () {
		if (plScrollTicking) return;
		plScrollTicking = true;
		requestAnimationFrame(function () {
			plScrollTicking = false;
			headerStyle();
			var nearBottom = window.scrollY + window.innerHeight >= document.documentElement.scrollHeight - 10;
			var backTop = document.getElementById('back-top');
			if (backTop) backTop.classList.toggle('show', nearBottom);
		});
	}, { passive: true });
...
awardBlocks.forEach((block) => {
  block.addEventListener(
    ...
    /* Back To Top: visibility handled in the throttled scroll listener above. */
  ...
  if ($(".progress-line").length) {
	...
	//Accordion Box
        ...
        // Paving Leads: the template returned from the whole IIFE here when a page had
        // no FAQ, which skipped WOW, nice-select and the video popup on those pages.
        if ($faqItems.length) {
