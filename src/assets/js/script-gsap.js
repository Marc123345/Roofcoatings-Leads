(function ($) {
    ...
    ================================ */
    // Paving Leads: GSAP ScrollSmoother moved the whole page (one huge transformed layer) with
    // JavaScript every frame. Native scrolling measured ~38% less script time and ~47% less
    // paint during a scroll, and browsers already ease wheel/trackpad scrolling themselves.
  ...
  /* ================================
   ...
   if (typeof gsap !== "undefined") {
        ...
        mm.add("(min-width: 1200px)", () => {
            ...
            // ===== tm-sub-tilte =====
            $('.tm-sub-tilte').each(function (index, el) {
            ...
            // Paving Leads: long statement headings have hundreds of characters; tweening
            // each one on scroll is heavy, so those animate word by word instead.
            let byWord = el.textContent.trim().length > 70;
            let split = new SplitText(el, {
                type: byWord ? "lines,words" : "lines,words,chars",
                linesClass: "split-line"
            });

            splits.push(split);
            split.chars = byWord ? split.words : split.chars;
