  ...
  return (
    <div className="space-y-2">
      {[
        ...
        <motion.div
          key={step.label}
          className={`${step.bg} rounded-none px-5 py-3 flex items-center justify-between ${step.text}`}
    ...
    <div className="space-y-4">
      {segments.map((seg) => (
        <div key={seg.label}>
          ...
          <div className="h-3 bg-white/10 rounded-none overflow-hidden">
            <motion.div
              className="h-full bg-gradient-to-r from-blue-light to-gold rounded-none"
              initial={{ width: 0 }}
...
function TimelineVisual() {
  ...
  return (
    <div className="relative">
      {/* Connecting line */}
      <motion.div
        className="absolute top-6 left-6 right-6 h-1 bg-white/10 rounded-none"
        initial={{ scaleX: 0 }}
        animate={{ scaleX: 1 }}
      ...
      />
      <motion.div
        className="absolute top-6 left-6 h-1 bg-gradient-to-r from-blue-light via-gold to-gold rounded-none"
        initial={{ width: 0 }}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 relative">
        {steps.map((step, i) => (
          ...
          >
            <motion.div
              className={`w-12 h-12 mx-auto rounded-none flex items-center justify-center text-xl relative z-10 ${
                i === 3
...
export default function WhyFacebookAds() {
  ...
  return (
    <section className="bg-navy py-16 sm:py-20 lg:py-32 relative overflow-hidden">
      ...
      <div className="absolute top-0 right-0 hidden sm:block w-[200px] h-[200px] lg:w-[500px] lg:h-[500px] bg-blue/5 rounded-none blur-[100px]" />
      <div className="absolute bottom-0 left-0 hidden sm:block w-[200px] h-[200px] lg:w-[400px] lg:h-[400px] bg-gold/3 rounded-none blur-[80px]" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <AnimateIn delay={0.2}>
          <div className="mt-12 flex justify-center">
            <div className="inline-flex bg-navy-light rounded-none p-1.5 border border-white/10">
              {tabs.map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id)}
                  className={`relative px-4 sm:px-6 py-2.5 sm:py-3 rounded-none text-xs sm:text-sm font-semibold transition-all duration-300 ${
                    activeTab === tab.id
                      ? "text-navy"
                ...
                >
                  {activeTab === tab.id && (
                    <motion.div
                      className="absolute inset-0 bg-gradient-to-r from-gold to-yellow-400 rounded-none"
                      layoutId="activeTab"
        ...
        <div className="mt-12">
          <AnimatePresence mode="wait">
            ...
            >
              {/* Left: stat + info */}
              <div className="bg-navy-light rounded-none p-5 sm:p-8 lg:p-10 border border-white/5">
                <motion.p
                ...
                <div className="mt-6 flex gap-4">
                  <div className="bg-white/5 rounded-none px-4 py-3 flex-1 text-center">
              ...
              <div className="bg-navy-light rounded-none p-5 sm:p-8 lg:p-10 border border-white/5 min-h-[280px] lg:min-h-[360px] flex flex-col justify-center">
        ...
        <StaggerContainer className="mt-12 grid grid-cols-2 lg:grid-cols-4 gap-4" staggerDelay={0.08}>
          {tabs.map((tab) => (
            <StaggerItem key={tab.id}>
              <motion.button
                onClick={() => setActiveTab(tab.id)}
                className={`w-full rounded-none p-5 text-center transition-all duration-300 border ${
                  activeTab === tab.id
