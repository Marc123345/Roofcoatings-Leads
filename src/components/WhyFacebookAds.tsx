      ...
      {segments.map((seg) => (
        <div key={seg.label}>
          ...
          <div className="h-3 bg-white/10 rounded-full overflow-hidden">
            <motion.div
              className="h-full bg-gradient-to-r from-blue-light to-gold rounded-full"
              initial={{ width: 0 }}
...
function TimelineVisual() {
  ...
  return (
    <div className="relative">
      {/* Connecting line */}
      <motion.div
        className="absolute top-6 left-6 right-6 h-1 bg-white/10 rounded-full"
        initial={{ scaleX: 0 }}
        animate={{ scaleX: 1 }}
      ...
      />
      <motion.div
        className="absolute top-6 left-6 h-1 bg-gradient-to-r from-blue-light via-gold to-gold rounded-full"
        initial={{ width: 0 }}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 relative">
        {steps.map((step, i) => (
          ...
          >
            <motion.div
              className={`w-12 h-12 mx-auto rounded-full flex items-center justify-center text-xl relative z-10 ${
                i === 3
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <AnimateIn delay={0.2}>
          <div className="mt-12 flex justify-center">
            <div className="inline-flex bg-navy-light rounded-2xl p-1.5 border border-white/10">
              {tabs.map((tab) => (
                <button
                  key={tab.id}
        ...
        <StaggerContainer className="mt-12 grid grid-cols-2 lg:grid-cols-4 gap-4" staggerDelay={0.08}>
          {tabs.map((tab) => (
            <StaggerItem key={tab.id}>
              <motion.button
                onClick={() => setActiveTab(tab.id)}
                className={`w-full rounded-xl p-5 text-center transition-all duration-300 border ${
                  activeTab === tab.id
