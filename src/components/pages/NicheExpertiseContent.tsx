    ...
    <div className="space-y-3">
      {rows.map((row, i) => (
        <motion.div key={row.label}
          className="bg-white/5 border border-white/5 rounded-none p-4"
          initial={{ opacity: 0, x: -15 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ delay: 0.15 + i * 0.1 }}>
          <div className="flex items-start gap-3">
            <div className="w-8 h-8 bg-white/10 rounded-none flex items-center justify-center shrink-0">{row.icon}</div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="inline-flex bg-navy-light rounded-none p-1.5 border border-white/10 gap-1 overflow-x-auto max-w-full scrollbar-none" style={{scrollbarWidth:"none"}}>
                {niches.map((n, i) => (
                  <button key={n.id} onClick={() => setActiveIdx(i)}
                    className={`relative px-4 sm:px-6 py-2.5 sm:py-3 rounded-none text-xs sm:text-sm font-semibold transition-all duration-300 ${activeIdx === i ? "text-navy" : "text-white/70 hover:text-white/70"}`}>
                    {activeIdx === i && (
                      <motion.div className={`absolute inset-0 bg-gradient-to-r ${n.color} rounded-none`}
            ...
            <AnimatePresence mode="wait">
              <motion.div key={niche.id}
                ...
                <div className="relative rounded-none overflow-hidden mb-8">
                  <div className="relative h-[280px] lg:h-[360px]">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-8 lg:p-10">
                      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
                        <div>
                          <motion.div className={`inline-flex items-center justify-center w-14 h-14 bg-white/10 rounded-none mb-3 text-white`}
                        ...
                        <div className="flex gap-3 shrink-0">
                          {niche.stats.map((stat) => (
                            <div key={stat.label} className="bg-navy/80 backdrop-blur-sm border border-white/10 rounded-none px-4 py-3 text-center min-w-[80px]">
                ...
                <div className="grid lg:grid-cols-3 gap-6">
                  ...
                  <div className="bg-navy-light rounded-none border border-white/10 p-5 sm:p-8 flex flex-col">
                    ...
                    <div className={`mt-6 bg-gradient-to-r ${niche.color} rounded-none p-5 text-center`}>
                  ...
                  {/* Campaign Breakdown */}
                  <div className="lg:col-span-2 bg-navy-light rounded-none border border-white/10 p-4 sm:p-6 lg:p-8">
                    ...
                    <motion.a href="/get-started"
                      className={`mt-6 w-full inline-flex items-center justify-center bg-gradient-to-r ${niche.color} text-white py-4 rounded-none font-bold text-lg shadow-lg`}
            ...
            {[
              ...
              <StaggerItem key={item.title}>
                <motion.div className="bg-white rounded-none p-5 sm:p-8 shadow-sm border border-transparent hover:border-blue/10 h-full"
                  whileHover={{ y: -4 }}>
                  <div className="w-10 h-10 bg-blue/10 rounded-none flex items-center justify-center">{item.icon}</div>
